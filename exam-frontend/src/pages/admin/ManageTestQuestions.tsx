...
const ManageTestQuestions = () => {
  ...
  const handleEditQuestion = (q: Question) => {
    ...
    setNewQuestion({
      ...
      Option1_GU: q.option1_GU || "",
      Option2_GU: q.option2_GU || "",
      Option3_GU: q.option3_GU || "",
      Option4_GU: q.option4_GU || "",
  ...
  const initData = useCallback(async () => {
    ...
    try {
      ...
      const currentTest = testsRes.data.find(
        (t: Question) => t.id === Number(testId),
      );
      if (currentTest) setTestName(currentTest.question_EN);
    } catch (err: unknown) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      !window.confirm(
        `Permanently remove ${selectedQuestions.length} selected items from the repository? This action cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `${selectedQuestions.length} items purged from registry.`,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Evaluation structure updated successfully.",
      });
      setTimeout(() => setMessage({ type: "", text: "" }), 4000);
    } catch (err: unknown) {
      console.error(err);
  ...
  const handleCreateQuestion = async (autoClose = true) => {
    try {
      if (editingId) {
        ...
        const createdId = (res.data as Question).id;
        ...
        setMessage({
          type: "success",
          text: `New question ${autoClose ? "created and linked." : "added successfully. Keep going!"}`,
        });
        setTimeout(() => setMessage({ type: "", text: "" }), 4000);
      }
  ...
  const handleDeleteQuestion = async (id: number) => {
    if (
      !window.confirm(
        "Permanently remove this item from the repository? This will also unassign it from any linked evaluations.",
      )
    )
      return;
              ...
              <Stack spacing={3}>
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", mt: 1 }}>
                    ...
                    <Typography
                      variant="caption"
                      sx={{ fontWeight: 700, color: "#EC4899" }}
                    >
                      Question items
                    </Typography>
                  </Box>
                ...
                >
                  ...
                  <Chip
                    ...
                    sx={{
                      bgcolor:
                        selectedQuestions.length > 0 ? "#10B981" : "#F59E0B",
                      color: "white",
                      fontWeight: 900,
                      fontSize: 10,
                    }}
            ...
            >
              ...
              >
                <Box>
                  ...
                  >
                    Select items to include in this evaluation phase
                  </Typography>
                </Box>
                <TextField
                  size="small"
              ...
              <Box sx={{ maxHeight: 800, overflow: "auto" }}>
                      ...
                      <Stack direction="row" spacing={2} alignItems="center">
                        <Chip
                          ...
                          sx={{
                            fontWeight: 900,
                            fontSize: 10,
                            bgcolor: isSelected ? "#EC4899" : "#F8FAFC",
                ...
                {filteredQuestions.length === 0 && (
                  <Box sx={{ p: 15, textAlign: "center" }}>
                    ...
                    <Typography sx={{ fontWeight: 800, color: "#ADB5BD" }}>
        ...
        <DialogTitle sx={{ bgcolor: "#0F172A", color: "white", py: 4, px: 5 }}>
          <Stack direction="row" alignItems="center" spacing={3}>
            ...
            <Box>
              ...
              >
                {editingId
                  ? `MODIFICATION PHASE - ID #${editingId}`
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Box>
                  ...
                  >
                    DEFINING THE CORRECT MCQ DATA POINT
                  </Typography>
                </Box>
              </Box>
              <TextField
              ...
              >
                {[1, 2, 3, 4].map((opt) => (
                  <MenuItem key={opt} value={opt} sx={{ fontWeight: 800 }}>
                    Option {opt}
                  </MenuItem>
                ))}
              </TextField>
