    ...
    >
      ...
      >
        <Container maxWidth={false} sx={{ px: { xs: 3, md: 6, lg: 10 } }}>
          ...
          {review && (
            ...
            >
              {review.userName} • {review.testName} • {review.score} /{" "}
              {review.totalQuestions} •{" "}
              {dayjs(review.submittedAt).format("MMM DD, YYYY [at] HH:mm")}
            </Typography>
          )}
        </Container>
      </Box>
      ...
      <Container maxWidth={false} sx={{ px: { xs: 3, md: 6, lg: 10 } }}>
        ...
        {!loading && !review && (
          ...
          >
            <Typography
              sx={{ fontWeight: 700, color: isDark ? "#CBD5E1" : "#64748B" }}
            >
          ...
          <>
                      ...
                      .map((item) => (
                        ...
                        >
                          ...
                          <TableCell sx={{ py: 2.5 }}>
                            ...
                            >{`Option ${item.correctOption}`}</Typography>
                            <Typography
                              variant="body2"
                              sx={{
                                color: isDark ? "#CBD5E1" : "#64748B",
            ...
            {review.items && review.items.length > pageSize && (
              ...
              >
                ...
                >
                  Showing{" "}
                  {Math.min((page - 1) * pageSize + 1, review.items.length)} -{" "}
                  {Math.min(page * pageSize, review.items.length)} of{" "}
                  {review.items.length}
                </Typography>
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setPage(
                        Math.min(
                          Math.ceil(review.items.length / pageSize),
                          page + 1,
                        ),
                      )
                    }
                    disabled={
                      page === Math.ceil(review.items.length / pageSize)
                    }
                    sx={{
                      minWidth: { xs: "32px", sm: "36px" },
                      p: { xs: 0.6, sm: 0.8 },
                  ...
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      setPage(Math.ceil(review.items.length / pageSize))
                    }
                    disabled={
                      page === Math.ceil(review.items.length / pageSize)
                    }
