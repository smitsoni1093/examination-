...
const TestList = () => {
  ...
  const handleEditImageUpload = (
    ...
    reader.onload = () => {
      setEditForm((prev) => ({
        ...prev,
        testImageUrl: reader.result as string,
      }));
  ...
  const fetchTests = async () => {
    ...
    try {
      ...
      if (Array.isArray(res.data)) {
        setTests(res.data as TestItem[]);
      }
    } catch (error: unknown) {
      console.error(error);
      setMessage({
        type: "error",
        text: "Synchronization with registry failed.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleStatus = async (testId: number, currentStatus: boolean) => {
    try {
      ...
      setMessage({
        type: "error",
        text: "Failed to update activation status.",
      });
    }
  };
  ...
  const handleOpenEdit = (test: TestItem) => {
    setEditForm({
      ...
      selectedInstructionIds: (test.instructions || []).map((item) => item.id),
    });
    setEditOpen(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (editForm.totalMarks <= 0) {
      setMessage({
        type: "error",
        text: "Total marks must be greater than zero.",
      });
      return;
    }
    ...
    if (!editForm.closingAt) {
      setMessage({
        type: "error",
        text: "Last date of submission is required.",
      });
      return;
    }
    ...
    setEditSaving(true);
    try {
      const payload: EditTestPayload = {
        name: editForm.name.trim(),
        description: editForm.description.trim(),
        duration: 0,
        totalMarks: Number(editForm.totalMarks),
        isGlobal: editForm.isGlobal,
        classId: editForm.isGlobal
          ? null
          : editForm.classId === ""
            ? null
            : Number(editForm.classId),
      ...
      setMessage({
        type: "success",
        text: "Test details updated successfully.",
      });
      await fetchTests();
    } catch (error: unknown) {
      ...
      setMessage({
        type: "error",
        text: serverMessage || "Failed to update test details.",
      });
    } finally {
      setEditSaving(false);
    }
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#F1F5F9", pb: 10 }}>
      ...
      <Container maxWidth={false} sx={{ px: { xs: 3, md: 6, lg: 10 } }}>
                ...
                <Paper
                  ...
                  sx={{
                    ...
                    "&:hover": {
                      ...
                      boxShadow: test.isActive
                        ? "0 25px 50px -12px rgba(236, 72, 153, 0.1)"
                ...
                >
                      ...
                      <Avatar
                        sx={{
                          bgcolor: test.isActive
                            ...
                            : "#F1F5F9",
                          color: test.isActive ? "#EC4899" : "#94A3B8",
                          borderRadius: 4,
                          width: 52,
                          height: 52,
                          flexShrink: 0,
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={() =>
                            handleToggleStatus(test.id, !!test.isActive)
                          }
                          color="secondary"
                        />
                      }
                  ...
                  <Chip
                    ...
                    label={
                      test.isGlobal
                        ? "Class: All Classes"
                        : `Class: ${test.className || "Unassigned"}`
                    }
                    sx={{
                      mb: 2,
                      fontWeight: 800,
                  ...
                  >
                    {test.description ||
                      "No detailed strategy provided for this evaluation."}
                  </Typography>
                  ...
                  >
                    ...
                    <Chip
                      icon={
                        <InfoOutlined sx={{ fontSize: "1rem !important" }} />
                      }
                  ...
                  <Button
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        bgcolor: "#EC4899",
                        boxShadow: "0 15px 25px -5px rgba(236, 72, 153, 0.3)",
                      },
                      transition: "all 0.3s",
        ...
        {!loading && tests.length === 0 && (
          ...
          >
            ...
            <Typography variant="h6" sx={{ fontWeight: 800, color: "#ADB5BD" }}>
              No active evaluations found.
            </Typography>
            <Button
              variant="text"
              startIcon={<Add />}
      ...
      >
        <DialogTitle sx={{ fontWeight: 900, px: { xs: 2, sm: 3 } }}>
          Edit Test Details
        </DialogTitle>
        <DialogContent sx={{ px: { xs: 2, sm: 3 } }}>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            ...
            >
              ...
              {editForm.testImageUrl && (
                <Button
                  color="error"
                  onClick={() =>
                    setEditForm((prev) => ({ ...prev, testImageUrl: null }))
                  }
                  sx={{ fontWeight: 700 }}
                >
                  Remove Image
            ...
            <TextField
              ...
              onChange={(e) =>
                setEditForm((prev) => ({ ...prev, closingAt: e.target.value }))
              }
              ...
              >
                <List sx={{ py: 0 }}>
                  {instructionBank
                    ...
                    .map((item) => {
                      const checked = editForm.selectedInstructionIds.includes(
                        item.id,
                      );
                      return (
                        <ListItem
                          ...
                          onClick={() =>
                            setEditForm((prev) => ({
                              ...
                              selectedInstructionIds: checked
                                ? prev.selectedInstructionIds.filter(
                                    (id) => id !== item.id,
                                  )
                  ...
                  {instructionBank.filter((item) => item.isActive).length ===
                    0 && (
                    <ListItem>
