...
import { Container, Typography, Box, Paper, InputAdornment, MenuItem, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Checkbox, IconButton, Chip, Stack, Avatar, CircularProgress, Alert, TextField, Button, Pagination } from '@mui/material';
import { Search, Delete, Visibility, Storage, Refresh, ArrowBack } from '@mui/icons-material';
...
const QuestionBank = () => {
    ...
    const fetchQuestionsPage = async (page: number, source: string, search: string) => {
        const safePage = Math.max(page, 1);
        const skip = (safePage - 1) * questionPageSize;
        const sourceParam = source === 'All Sources' ? '' : source;

        const res = await adminApi.getQuestionsPaged(sourceParam, search, skip, questionPageSize);
        const items = Array.isArray(res.data?.items)
            ? res.data.items
            : (Array.isArray(res.data) ? res.data : []);
        const totalCount = typeof res.data?.totalCount === 'number'
            ? res.data.totalCount
            : items.length;
    ...
    const initData = async () => {
        ...
        try {
            ...
            setSources((sRes.data ?? []).filter((s: any) => s !== null));
            ...
            try {
                ...
                setSourceFiles([]);
                setMessage({ type: 'error', text: 'Imported source files could not be loaded. Please restart backend and refresh.' });
            }
        } catch (err) {
            console.error(err);
    ...
    const handleFilterChange = async (source: string, search: string, page: number = 1) => {
        setLoading(true);
        try {
    ...
    const handleBulkDelete = async () => {
        if (!window.confirm(`Permanently remove ${selectedIds.length} items from the repository?`)) return;
        try {
    ...
    const handleSoftDeleteSource = async (sourceFileName: string) => {
        if (!window.confirm(`Soft delete source file ${sourceFileName}? This hides its questions from active source lists.`)) return;
        try {
            await adminApi.softDeleteQuestionSource(sourceFileName);
            setMessage({ type: 'success', text: 'Source file soft deleted successfully.' });
            await initData();
            await handleFilterChange(sourceFilter, searchTerm, questionPage);
        } catch (err: any) {
            setMessage({ type: 'error', text: err?.response?.data?.message || 'Unable to delete source file.' });
        }
    };
    ...
    return (
        ...
        >
            {/* Header Area */}
            <Box sx={{ bgcolor: isDark ? '#000000' : '#FFFFFF', borderBottom: '1px solid #E2E8F0', py: 5, mb: 6 }}>
                    ...
                    <Stack direction="row" justifyContent="space-between" alignItems="flex-end">
                        <Box>
                            <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 1.5 }}>
                                ...
                                <Typography variant="overline" sx={{ fontWeight: 800, color: isDark ? '#CBD5E1' : '#94A3B8', letterSpacing: 1.5 }}>Inventory Management</Typography>
                            </Stack>
                            <Typography variant="h3" sx={{ fontWeight: 900, letterSpacing: '-1.5px', color: isDark ? '#FFFFFF' : '#0F172A' }}>Global Question Bank</Typography>
                        </Box>
                        ...
                        <Stack direction="row" spacing={2}>
                            <Button
                                ...
                                sx={{ fontWeight: 700, borderRadius: 3, color: isDark ? '#FFFFFF' : undefined, borderColor: isDark ? 'rgba(148, 163, 184, 0.45)' : undefined }}
                            >
                                Back
                            </Button>
                             {selectedIds.length > 0 && (
                                <IconButton color="error" onClick={handleBulkDelete} sx={{ bgcolor: isDark ? '#000000' : 'rgba(239, 68, 68, 0.05)', borderRadius: 3, px: 2 }}>
                                    <Delete sx={{ mr: 1 }} />
                                    <Typography variant="button" sx={{ fontWeight: 800 }}>Delete ({selectedIds.length})</Typography>
                                </IconButton>
                            )}
                            <IconButton onClick={() => handleFilterChange(sourceFilter, searchTerm, questionPage)} sx={{ bgcolor: isDark ? '#000000' : '#F8FAFC', borderRadius: 3 }}>
                                <Refresh />
                            </IconButton>
                        </Stack>
                    </Stack>
            ...
            <Container maxWidth={false} sx={{ px: { xs: 4, md: 8, lg: 12 } }}>
                ...
                <Paper elevation={0} sx={{ borderRadius: 6, border: '1px solid #E2E8F0', overflow: 'hidden', mb: 4 }}>
                    <Box sx={{ p: 3, bgcolor: isDark ? '#000000' : '#FFFFFF', borderBottom: '1px solid #F1F5F9' }}>
                        <Typography variant="h6" sx={{ fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A' }}>Imported Source Files</Typography>
                        <Typography variant="body2" sx={{ color: isDark ? '#CBD5E1' : '#64748B', mt: 0.5 }}>
                            You can soft delete a file only when none of its questions are assigned to tests.
                        </Typography>
                    </Box>
                    ...
                    <TableContainer>
                        <Table>
                            <TableHead>
                                <TableRow>
                                    <TableCell sx={{ fontWeight: 900, color: isDark ? '#FFFFFF' : '#64748B', bgcolor: isDark ? '#000000' : '#F8FAFC' }}>FILE NAME</TableCell>
                                    <TableCell sx={{ fontWeight: 900, color: isDark ? '#FFFFFF' : '#64748B', bgcolor: isDark ? '#000000' : '#F8FAFC' }}>QUESTIONS</TableCell>
                                    <TableCell sx={{ fontWeight: 900, color: isDark ? '#FFFFFF' : '#64748B', bgcolor: isDark ? '#000000' : '#F8FAFC' }}>IN USE</TableCell>
                                    <TableCell sx={{ fontWeight: 900, color: isDark ? '#FFFFFF' : '#64748B', bgcolor: isDark ? '#000000' : '#F8FAFC' }}>LAST IMPORT</TableCell>
                                    <TableCell align="right" sx={{ fontWeight: 900, color: isDark ? '#FFFFFF' : '#64748B', bgcolor: isDark ? '#000000' : '#F8FAFC' }}>ACTIONS</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {sourceFiles.map((sf) => (
                                    <TableRow key={sf.sourceFileName} hover sx={{ '&:hover': { bgcolor: isDark ? '#111111' : undefined } }}>
                                        <TableCell sx={{ fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A' }}>{sf.sourceFileName}</TableCell>
                                        ...
                                        <TableCell>
                                            <Typography variant="body2" sx={{ color: isDark ? '#CBD5E1' : '#64748B', fontWeight: 600 }}>
                                                {new Date(sf.lastImportedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                            </Typography>
                                        </TableCell>
                                        <TableCell align="right">
                                            <Button
                                                ...
                                                onClick={() => handleSoftDeleteSource(sf.sourceFileName)}
                                            >
                                                Soft Delete
                                            </Button>
                                        </TableCell>
                                ...
                                {sourceFiles.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={5} sx={{ py: 5, textAlign: 'center' }}>
                                            <Typography sx={{ fontWeight: 700, color: isDark ? '#CBD5E1' : '#94A3B8' }}>No imported files found.</Typography>
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                ...
                <Paper elevation={0} sx={{ borderRadius: 6, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
                    {/* Filters Header */}
                    <Box sx={{ p: 4, bgcolor: isDark ? '#000000' : '#FFFFFF', borderBottom: '1px solid #F1F5F9', display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                        <TextField
                            size="small"
                        ...
                        <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
                            <Box sx={{ textAlign: 'right' }}>
                                <Typography variant="caption" sx={{ color: isDark ? '#CBD5E1' : '#94A3B8', fontWeight: 800, display: 'block' }}>POOL SIZE</Typography>
                                <Typography variant="h6" sx={{ fontWeight: 900 }}>{totalQuestions}</Typography>
                            </Box>
                            <Box sx={{ textAlign: 'right' }}>
                                <Typography variant="caption" sx={{ color: isDark ? '#CBD5E1' : '#94A3B8', fontWeight: 800, display: 'block' }}>RESOURCES</Typography>
                                <Typography variant="h6" sx={{ fontWeight: 900 }}>{sources.length}</Typography>
                            </Box>
                        </Box>
                    </Box>
                            ...
                            <Table stickyHeader sx={{ minWidth: { xs: 700, sm: 900 } }}>
                                ...
                                <TableBody>
                                    {questions.map((q) => {
                                        ...
                                        return (
                                            <TableRow key={q.id} hover selected={isSelected} sx={{ '&:hover': { bgcolor: isDark ? '#111111 !important' : '#F8FAFC !important' } }}>
                                                <TableCell padding="checkbox">
                                                    <Checkbox checked={isSelected} onChange={() => toggleSelect(q.id)} />
                                                </TableCell>
                                                <TableCell sx={{ py: 3 }}>
                                                    <Typography sx={{ fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A', mb: 0.5 }}>{q.question_EN}</Typography>
                                                    <Stack direction="row" spacing={1}>
                                                        <Typography sx={{ color: isDark ? '#CBD5E1' : '#94A3B8', fontSize: 11, fontWeight: 800 }}>#Q-{q.id.toString().padStart(4, '0')}</Typography>
                                                ...
                                                <TableCell align="right">
                                                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                                                        <IconButton size="small" sx={{ color: isDark ? '#E2E8F0' : '#94A3B8' }}><Visibility sx={{ fontSize: 18 }} /></IconButton>
                                                        <IconButton size="small" sx={{ color: isDark ? '#E2E8F0' : '#94A3B8' }}><Delete sx={{ fontSize: 18 }} /></IconButton>
                                                    </Stack>
                                                </TableCell>
                                            </TableRow>
                                        );
                                    ...
                                    {questions.length === 0 && (
                                        <TableRow>
                                            <TableCell colSpan={5} sx={{ py: 15, textAlign: 'center' }}>
                                                <Typography sx={{ fontWeight: 800, color: isDark ? '#CBD5E1' : '#ADB5BD' }}>Repository currently empty.</Typography>
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
