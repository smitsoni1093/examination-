...
import { CloudUpload, TableChart, CheckCircle, ErrorOutline, FileDownload, ArrowBack, Storage, Preview } from '@mui/icons-material';
...
const ImportQuestions = () => {
    ...
    const validateMapping = () => {
        const requiredFields: Array<keyof MappingState> = ['question', 'optionA', 'optionB', 'correctAnswer'];
    ...
    const handlePreview = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Preview failed. Please review the mapping and try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const downloadTemplate = () => {
        const csvHeaders = 'Column 1,Column 2,Column 3,Column 4,Column 5,Column 6,Column 7\n';
        const example = '1,What is the capital of France?,Paris,London,Berlin,Madrid,A';
    ...
    const renderFieldSelect = (fieldKey: keyof MappingState, label: string) => {
        ...
        return (
            <TableRow key={fieldKey} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                <TableCell sx={{ width: '40%', py: 2.25 }}>
                    <Typography sx={{ fontWeight: 800, color: '#0F172A' }}>{label}</Typography>
                </TableCell>
                <TableCell sx={{ width: '60%', py: 2.25 }}>
                    <FormControl fullWidth size="small">
                        ...
                        <Select
                            ...
                            onChange={(event: SelectChangeEvent<string>) => handleMappingChange(fieldKey, event.target.value)}
                        >
                            <MenuItem value="">
    ...
    const renderPreviewStatus = (row: PreviewRow) => (
        <Chip
            ...
            sx={{
                fontWeight: 900,
                borderRadius: 2,
                bgcolor: row.isValid ? 'rgba(16, 185, 129, 0.12)' : 'rgba(239, 68, 68, 0.12)',
                ...
                <Grid container spacing={4}>
                    <Grid item xs={12}>
                        ...
                        >
                            ...
                            <Typography variant="h5" sx={{ fontWeight: 800, mb: 1 }}>Manual Question Import</Typography>
                            <Typography variant="body1" sx={{ color: '#64748B', mb: 2 }}>
                                Upload any .xlsx or .csv file. You will map every system field manually before import.
                            </Typography>
                            ...
                            {file && (
                                <Stack spacing={2} sx={{ mt: 4, alignItems: 'center' }}>
                                    <Box sx={{ p: 2, bgcolor: '#F1F5F9', borderRadius: 3, display: 'inline-flex', alignItems: 'center' }}>
                                    ...
                                    <Button 
                                        ...
                                        sx={{ bgcolor: '#6366F1', fontWeight: 900, borderRadius: 2, px: 5 }}
                                    >
                            ...
                            {loading && (
                                <Box sx={{ mt: 4, width: '100%' }}>
                                    ...
                                    <Typography variant="caption" sx={{ mt: 1, display: 'block', fontWeight: 700, color: '#64748B' }}>Parsing file and preparing mapping session...</Typography>
                                </Box>
                            )}
                        </Paper>
                    </Grid>
                        ...
                        <Grid item xs={12}>
                            <Paper elevation={0} sx={{ p: 4, borderRadius: 6, border: '1px solid #E2E8F0', bgcolor: '#FFFFFF' }}>
                                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                                    <Box>
                                        <Typography variant="h6" sx={{ fontWeight: 800 }}>Manual Column Mapping</Typography>
                                        <Typography variant="body2" sx={{ color: '#64748B' }}>
                                            Template field on the left, Excel header selector on the right.
                                        </Typography>
                                    </Box>
                                    <Chip label={`${headers.length} uploaded columns`} sx={{ fontWeight: 800 }} />
                                </Stack>
                                    ...
                                    <Table size="small">
                                        <TableHead>
                                            <TableRow sx={{ bgcolor: '#F8FAFC' }}>
                                                <TableCell sx={{ fontWeight: 900, color: '#475569', width: '40%' }}>Template Side</TableCell>
                                                <TableCell sx={{ fontWeight: 900, color: '#475569', width: '60%' }}>Excel Side</TableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {systemFields.map((field) => renderFieldSelect(field.key, field.label))}
                                        </TableBody>
                                    </Table>
                                </TableContainer>
                                ...
                                <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
                                    <Button variant="outlined" onClick={restartImport} sx={{ fontWeight: 800 }}>
                                        Start Over
                                    </Button>
                                    <Box sx={{ flex: 1 }} />
                                    <Button variant="contained" startIcon={<Preview />} onClick={handlePreview} sx={{ bgcolor: '#0F172A', fontWeight: 900 }}>
                                        Preview Rows
                                    </Button>
                                </Stack>
                            </Paper>
                    ...
                    {step === 'preview' && preview && (
                        <Grid item xs={12}>
                            <Paper elevation={0} sx={{ p: 4, borderRadius: 6, border: '1px solid #E2E8F0', bgcolor: '#FFFFFF' }}>
                                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                                    <Box>
                                        <Typography variant="h6" sx={{ fontWeight: 800 }}>Preview</Typography>
                                        <Typography variant="body2" sx={{ color: '#64748B' }}>
                                            Review the mapped rows before final import.
                                        </Typography>
                                    </Box>
                                    <Chip label={`${preview.totalRows} rows`} sx={{ fontWeight: 800 }} />
                                </Stack>

                                <Stack direction="row" spacing={3} sx={{ mb: 4 }}>
                                    <Box>
                                        <Typography variant="caption" sx={{ fontWeight: 800, color: '#94A3B8' }}>VALID</Typography>
                                        <Typography variant="h4" sx={{ fontWeight: 900, color: '#10B981' }}>{preview.validRows}</Typography>
                                    </Box>
                                    <Box>
                                        <Typography variant="caption" sx={{ fontWeight: 800, color: '#94A3B8' }}>INVALID</Typography>
                                        <Typography variant="h4" sx={{ fontWeight: 900, color: '#EF4444' }}>{preview.invalidRows}</Typography>
                                    </Box>
                                </Stack>

                                <TableContainer sx={{ border: '1px solid #E2E8F0', borderRadius: 3, mb: 3 }}>
                                    <Table size="small">
                                        <TableHead>
                                            <TableRow sx={{ bgcolor: '#F8FAFC' }}>
                                                ...
                                                <TableCell sx={{ fontWeight: 800 }}>Correct Answer</TableCell>
                                        ...
                                        <TableBody>
                                            {paginatedPreviewRows.map((row) => (
                                                <TableRow key={row.rowNumber} hover>
                                                    ...
                                                    <TableCell>{row.values['Correct Answer'] || '-'}</TableCell>
                                ...
                                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} sx={{ mb: 2 }}>
                                    ...
                                    <Stack direction="row" spacing={1} alignItems="center" justifyContent={{ xs: 'space-between', sm: 'flex-end' }}>
                                        ...
                                        <Button
                                            size="small"
                                            variant="outlined"
                                            onClick={() => setPreviewPage((prev) => Math.min(previewPageCount - 1, prev + 1))}
                                    ...
                                    <List sx={{ bgcolor: 'rgba(239, 68, 68, 0.02)', borderRadius: 4, p: 2 }}>
                                        {preview.errors.map((item, index) => (
                                            <ListItem key={`${item}-${index}`}>
                                                ...
                                                <ListItemText primary={item} primaryTypographyProps={{ fontWeight: 600, color: '#64748B' }} />
                                            </ListItem>
                                        ))}
                                    </List>
                                )}
                                ...
                                <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
                                    <Button variant="outlined" onClick={() => setStep('mapping')} sx={{ fontWeight: 800 }}>
                                        Back to Mapping
                                    </Button>
                                    <Box sx={{ flex: 1 }} />
                                    <Button variant="contained" onClick={handleConfirm} sx={{ bgcolor: '#0F172A', fontWeight: 900 }}>
                                        Confirm Import
                                    </Button>
                                </Stack>
                            </Paper>
                        ...
                        <Grid item xs={12}>
                            <Paper elevation={0} sx={{ p: 4, borderRadius: 6, border: '1px solid #E2E8F0', bgcolor: '#FFFFFF' }}>
                                <Stack direction="row" spacing={4} sx={{ mb: 6 }}>
                                    ...
                                    <Box>
                                        <Typography variant="caption" sx={{ fontWeight: 800, color: '#F59E0B' }}>SKIPPED</Typography>
                                        <Typography variant="h3" sx={{ fontWeight: 900, color: '#F59E0B' }}>{result.skippedCount}</Typography>
                                    </Box>
                                </Stack>
                                ...
                                {result.errors.length > 0 ? (
                                    ...
                                    <Box sx={{ p: 4, textAlign: 'center', bgcolor: 'rgba(16, 185, 129, 0.05)', borderRadius: 4 }}>
                                        ...
                                        <Typography variant="body1" sx={{ fontWeight: 700, color: '#047857' }}>All valid rows were imported successfully.</Typography>
                                    </Box>
                                )}
